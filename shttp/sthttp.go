...
func (stClient *Client) GetConfig() (c Config, err error) {
	...
	fmt.Println("timeout  GetConfig", stClient.HTTPConfig.HTTPTimeout)
...
func (stClient *Client) GetServers() (servers []Server, err error) {
	...
	fmt.Println("timeout  GetServers", stClient.HTTPConfig.HTTPTimeout)
...
func (stClient *Client) GetFastestServer(servers []Server) Server {
	...
	for server := range servers {
		...
		if err != nil {
			...
			fmt.Println("Error GetLatency", err)
			latency = 0.0
			//log.Fatal(err)
		}
...
func (stClient *Client) DownloadSpeed(url string) (speed float64, err error) {
	...
	client, err := stClient.getHTTPClient()
	if err != nil {
		fmt.Println("DownloadSpeed getHTTPClient Error")
		return 0, err
	}
	...
	resp, err := client.Do(req)
	if err != nil {
		fmt.Println("DownloadSpeed Error", err)
		return 0, err
	}
	defer resp.Body.Close()
	...
	body, err := ioutil.ReadAll(resp.Body)
	if err != nil {
		fmt.Println("DownloadSpeed ioutil.ReadAll Error", err)
		return 0, err
	}
	bodyLen := len(body)
	finish := time.Now()
...
func (stClient *Client) UploadSpeed(url string, mimetype string, data []byte) (speed float64, err error) {
	...
	client, err := stClient.getHTTPClient()
	if err != nil {
		fmt.Println("UploadSpeed getHTTPClient Error", err)
		return 0, err
	}
	resp, err := client.Post(url, mimetype, buf)
	finish := time.Now()
	if err != nil {
		fmt.Println("UploadSpeed Post Error", err)
		return 0, err
	}
	...
	if err != nil {
		fmt.Println("UploadSpeed ioutil.ReadAll Error", err)
		return 0, err
	}
